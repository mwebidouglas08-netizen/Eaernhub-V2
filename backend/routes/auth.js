...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    await dbRun(
      `INSERT INTO users (username,email,country,mobile,password,referral_code,referred_by)
       VALUES (?,?,?,?,?,?,?)`,
      [username.trim(), email.trim().toLowerCase(),
       country || 'Kenya', mobile, hash, refCode, referral || null]
    );
    if (referral) {
      const referrer = await dbGet(
        'SELECT id FROM users WHERE referral_code=?', [referral]
      );
      if (referrer) {
        ...
        await dbRun(
          'UPDATE users SET affiliate_earnings=affiliate_earnings+?, balance=balance+? WHERE id=?',
...
router.post('/activate', async (req, res) => {
  ...
  try {
    ...
    await dbRun(
      'INSERT INTO payments (user_id,amount,phone,type,status) VALUES (?,?,?,?,?)',
      [req.session.userId, fee, phone, 'activation', 'completed']
    );
