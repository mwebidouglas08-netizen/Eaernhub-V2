...
router.post('/withdraw', requireActivated, async (req, res) => {
  ...
  try {
    ...
    await dbRun(
      'INSERT INTO withdrawals (user_id,amount,mobile) VALUES (?,?,?)',
      [req.session.userId, amt, mobile]
    );
...
router.get('/downlines', requireActivated, async (req, res) => {
  const user = await dbGet(
    'SELECT referral_code FROM users WHERE id=?', [req.session.userId]
  );
  const downlines = await dbAll(
    'SELECT username,country,created_at FROM users WHERE referred_by=? ORDER BY created_at DESC',
    [user?.referral_code || '']
  );
...
router.post('/spin', requireActivated, async (req, res) => {
  ...
  if (prize > 0) {
    await dbRun(
      ...
      [prize, prize, req.session.userId]
    );
  }
  return res.json({
    success: true, prize,
    message: prize > 0 ? `🎉 You won KES ${prize}!` : 'Better luck next time!'
  });
});
